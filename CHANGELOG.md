# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [1.0.0] - 2026-03-23

### Added

- CLI with `install`, `uninstall`, `test`, `notify`, `status`, `preset`, `sound`, and `reset` commands
- Cross-platform sound playback (Windows, macOS, Linux)
- Five built-in presets: `default`, `gentle`, `urgent`, `chime`, `pulse`
- Custom `.wav` file support
- Automatic Claude Code hook installation via `claude-bell install`
- User configuration stored at `~/.claude-bell/config.json`

[1.0.0]: https://github.com/GuxtavoLiu/claude-bell/releases/tag/v1.0.0
